...
# created_at
# committer_id  addtions    deletitions  created_at
...
data_commit = cursor.fetchall()
last_date_commits = data_commit[-1][5].date()
if (last_date_commits-last_date).days>0:
    last_date = last_date_commits
data_commits = []
for i in range(len(data_commit)):
    data_commit_codeline_committer_one = []
    # committer_id
    data_commit_codeline_committer_one.append(data_commit[i][3])
    
    # addition  deletition
    datapat = re.compile(r'\d+')
    session = HTMLSession()
    url = base_url + "/commit/" + data_commit[i][1]
    # print(url)
    r = session.get(url)
    
    r_content = r.html.find("#toc > div.toc-diff-stats")
    try:
        # print(r_content[0].text)
        codeline = datapat.findall(r_content[0].text)
    except IndexError:
        print("Error: " + url)
    else:
        data_commit_codeline_committer_one.append(int(codeline[-2]))
        data_commit_codeline_committer_one.append(int(codeline[-1]))
        #created_at
        data_commit_codeline_committer_one.append(data_commit[i][5])
        # print(data_commit_codeline_committer_one)
        data_commits.append(tuple(data_commit_codeline_committer_one))
        if i == len(data_commit)-1:
            print("get data commit")
            print("last commit is :")
            print(data_commit_codeline_committer_one)
            break
...
headers = ['project_id','date', 'forks','committer_id',"addition",'deletition','commits','commit_comment',
...
with open('getdata.csv','w') as f:
    ...
    for i in range(span+1):
        ...
        dels = 0
        commits = 0
        while cur_row_commit<length_commit and (data_commits[cur_row_commit][3].date()-today).days<=0:
            if (data_commits[cur_row_commit][3].date()-today).days==0:
                adds += data_commits[cur_row_commit][1]
                dels += data_commits[cur_row_commit][2]
                commits += 1
                person_id = data_commits[cur_row_commit][0]
                if person_id in committer:
                    committer[person_id][0] += data_commits[cur_row_commit][1]
                    committer[person_id][1] += data_commits[cur_row_commit][2]
                else:
                    committer[person_id] = [data_commits[cur_row_commit][1],data_commits[cur_row_commit][2]]
            cur_row_commit += 1

        # get commit comment
        commit_comment = 0
        while cur_row_commit_comment<length_commit_comment and (data_commit_comment[cur_row_commit_comment][13].date()-today).days<=0:
        ...
        # get issue comment
        issue_comment = 0
        while cur_row_issue_comment<length_issue_comment and (data_issue_comment[cur_row_issue_comment][11].date()-today).days<=0:
...
# headers = ['project_id','date', 'forks','committer_id',"addition",'deletition','commits','commit_comment',
#            'req_opened','req_closed','req_merged','other','issue','issue_comment','watchers']
        one_data = (project_id, today, forks, committer, adds, dels, commits, commit_comment, pull_request_opened,
